...
const BASE_URL = process.env.UI_CAPTURE_URL || "http://127.0.0.1:4173";
const OUTPUT_DIR = path.resolve(process.cwd(), "docs/ui-overhaul-captures/phase5");
...
const SURFACES = [
  ...
  { key: "saved-views", selector: "#saved-views-card" },
  { key: "search", selector: "#search-panel" },
  { key: "analytics", selector: "#participants" },
];
...
async function captureRun(browser, plan) {
  ...
  for (const surface of SURFACES) {
    ...
    const name = `phase5-${plan.scheme}-${plan.width}-${surface.key}.png`;
...
async function main() {
  ...
  process.stdout.write(
    `Captured ${CAPTURE_PLAN.length * SURFACES.length} screenshots in docs/ui-overhaul-captures/phase5\n`,
  );
}
