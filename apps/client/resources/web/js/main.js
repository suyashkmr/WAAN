    ...
    return {
      ...
      running:
        typeof raw.running === "boolean"
          ? raw.running
          : state === "running",
      startedAt: raw.startedAt ?? payload.startedAt ?? null,
      readyAt: raw.readyAt ?? payload.readyAt ?? null,
      stoppedAt: raw.stoppedAt ?? payload.stoppedAt ?? null,
      pid: raw.pid ?? payload.pid ?? null,
      version: raw.version ?? payload.version ?? null,
      lastError: raw.lastError ?? payload.lastError ?? null,
    };
  }
  const state =
...
function updateRelayStatusDisplay(payload) {
  if (!relayStatusIndicator) return;
  const status = normalizeRelayStatusPayload(payload);
  if (status.running) {
    setRelayIndicator("Running", "status-running");
  } else if (status.state === "starting") {
    setRelayIndicator("Starting…", "status-connecting");
  } else if (status.state === "error") {
    setRelayIndicator("Error", "status-stopped");
  } else {
  ...
  if (relayStatusMeta) {
    ...
    if (status.startedAt && status.running) {
      parts.push(`since ${formatTimestampDisplay(status.startedAt)}`);
    } else if (!status.running && status.stoppedAt) {
      parts.push(`last stopped ${formatTimestampDisplay(status.stoppedAt)}`);
    }
