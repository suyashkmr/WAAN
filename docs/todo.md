...
- [ ] Phase 12 (3-5 days): Prime theme tokens as single source of truth.
  - [ ] Adopt `@primeuix/themes` tokens as canonical design tokens for component visuals.
    ...
    - [ ] Record any intentionally retained non-Prime tokens and justify why they are app-frame/layout tokens instead of component-skinning tokens.
      - [x] Audit note: redundant pass-through control aliases in `styles/components/search-saved.css`, the `--highlight-accent` layer in `styles/components/analytics.css`, and analytics shell recipe aliases have been removed; the old analytics-only `--card-accent` wrapper was replaced with a narrower `--analytics-accent` alias so analytics panels can keep local accents without colliding with the shared shell `--section-accent` contract.
      - [x] Documented the intent in `docs/design-tokens.md`: migrated component stylesheets should not retain broad wrapper-token systems, but analytics surfaces may use the scoped `--analytics-accent` alias where panel-local accent isolation is required.
      - [x] Component stylesheet audit note: the remaining `styles/components/*.css` layer no longer contains a parallel raw color/shadow palette; the last no-op raw literal in `styles/components/navigation.css` was removed, and the only justified remaining component-local alias is the narrowly scoped analytics accent isolation token.
