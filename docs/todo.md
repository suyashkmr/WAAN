...
- [ ] Phase 12 (3-5 days): Prime theme tokens as single source of truth.
  - [ ] Adopt `@primeuix/themes` tokens as canonical design tokens for component visuals.
    ...
    - [ ] Record any intentionally retained non-Prime tokens and justify why they are app-frame/layout tokens instead of component-skinning tokens.
      - [x] Audit note: current remaining component-local custom properties in `styles/components/*.css` are mostly semantic aliases (`--card-accent`, analytics shell helpers) rather than parallel raw palette systems; redundant pass-through control aliases in `styles/components/search-saved.css` have been removed and the next sweep should remove or justify each remaining alias explicitly.
      - [x] Documented the remaining intentional semantic wrappers in `docs/design-tokens.md`: `--card-accent` for per-surface accent assignment and `--analytics-shell-*` for analytics shell recipe composition. These are now explicitly treated as allowed wrappers over shared Prime-backed root tokens, not a second palette system.
