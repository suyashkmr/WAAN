...
- [ ] Phase 12 (3-5 days): Prime theme tokens as single source of truth.
  - [ ] Adopt `@primeuix/themes` tokens as canonical design tokens for component visuals.
    ...
    - [ ] Record any intentionally retained non-Prime tokens and justify why they are app-frame/layout tokens instead of component-skinning tokens.
      - [x] Audit note: current remaining component-local custom properties in `styles/components/*.css` are narrowed to the single semantic accent wrapper `--card-accent`; redundant pass-through control aliases in `styles/components/search-saved.css`, the `--highlight-accent` layer in `styles/components/analytics.css`, and analytics shell recipe aliases have been removed.
      - [x] Documented the remaining intentional semantic wrapper in `docs/design-tokens.md`: `--card-accent` for per-surface accent assignment. Analytics shell recipe aliases and the highlight-specific accent wrapper are no longer needed because those surfaces now consume shared root tokens directly.
      - [x] Component stylesheet audit note: the remaining `styles/components/*.css` layer no longer contains a parallel raw color/shadow palette; the last no-op raw literal in `styles/components/navigation.css` was removed, leaving `--card-accent` as the only intentional component-local semantic wrapper.
