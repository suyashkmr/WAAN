...
- [ ] Phase 14 (2-4 days): Final 100% migration sign-off, PrimeVue completion, and purge.
  - [ ] Complete the deferred PrimeVue form-control adoption so the remaining core controls are no longer native fallbacks.
    - [x] Restore PrimeVue `Select/Dropdown` as the production default where runtime-native `select` is still used.
      ...
      - [ ] Remaining blocker: page controls (`#chat-selector`, `#global-range`) are still intentionally native because their current controller/event contracts mutate cached DOM refs directly. Dashboard participant controls now use PrimeVue-visible bridged selects while preserving their native refs (`js/vue/dashboardParticipantControlBridge.js`, `js/vue/dashboardPanelsIsland.js`), but the page shell still needs the same treatment before this line can be considered fully complete.
    - [x] Restore PrimeVue `DatePicker/Calendar` as the production default where runtime-native `input[type="date"]` is still used.
      ...
      - [ ] Remaining blocker: page custom-range controls (`#custom-start`, `#custom-end`) are still intentionally native because the page-control shell bridge and detached listener flows still depend on real native date inputs. They need a bridged PrimeVue date path that preserves those DOM contracts before full completion.
