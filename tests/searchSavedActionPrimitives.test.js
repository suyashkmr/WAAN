...
describe("search saved action primitives", () => {
  ...
  it("binds bridge-owned search form submit without requiring a deleted actions wrapper", () => {
    document.body.innerHTML = `
    ...
    expect(document.querySelector("#advanced-search-form .search-actions")).toBeNull();
    form?.dispatchEvent(new Event("submit", { bubbles: true, cancelable: true }));
    expect(dispatchPanelAction).toHaveBeenCalledWith("search:run-search");
  });

  it("does not replace shipped direct search buttons when the deleted actions wrapper is absent", () => {
    ...
    expect(runButton?.getAttribute("data-ui-runtime")).toBeNull();
    expect(resetButton?.getAttribute("data-ui-runtime")).toBeNull();
  });
